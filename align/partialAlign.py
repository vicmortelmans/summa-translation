...
def posetDump(chain, pairs, huLines, enLines, outputFilename) :
    ...
    dropFile = open(outputFilename+".poset","w")
    chainSet = set(chain)
    for p in pairs :
...
def partialAlignWithIO(huFilename, enFilename, outputFilename, huLangName, enLangName, maximalChunkSize) :
    log("Reading corpora...")
    huLines = [l.strip("\n") for l in open(huFilename).readlines()]
    enLines = [l.strip("\n") for l in open(enFilename).readlines()]
    log("Done.")
